...
<script>
  export default {
    ...
    computed: {
      ...
      tagsList() {
        // console.log("pageOpen"+this.$store.state.app.pageOpenedList)
    ...
    methods: {
      ...
      closePage(e, name) {
        ...
        if (this.currentPageName === name) {
          ...
          if (this.pageTagsList.length === 1) {
            ...
            // var activesp = pagePath.replace('//','/')
            // var ap = activesp.split('/')[0]
            // console.log(ap)
            // this.$router.push(ap);
            pagePath = this.pageTagsList[1].path;
            var activesp = pagePath.replace('//','/')
            // console.log(activesp)
            // var ap = pagePath.split('//')[1]
      ...
      linkTo(item) {
        ...
        // console.log(routerObj.path)
        var iPath;
          // console.log(item.path);
        var ppath = item.path.split('/')[1]
        iPath = item.path.split('//')[1];
      
        if(!iPath){
          // console.log(1)
          // console.log(ppath)
          return
          this.$emit("on-select",ppath);

          // this.$router.push("/home");
        }
        else {
          // console.log(3)
          this.$emit("on-select",iPath);
        }
...
<template>
  <div ref="scrollCon" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll" class="tags-outer-scroll-con">
    ...
    <div ref="scrollBody" class="tags-inner-scroll-body" :style="{left: tagBodyLeft + 'px'}">
      <transition-group name="taglist-moving-animation" tag="p">
        <Tag
          type="dot"
          v-for="(item, index) in pageTagsList"
